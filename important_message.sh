...
# Slackに関する設定
channel_urlencoding=$(echo "シェルスクリプト連載" | python3 -c "import sys, urllib.parse; print (urllib.parse.quote(sys.stdin.read().rstrip()));")
token="トークン"
...
# 重要なメッセージをSlackに転送
  if [ ${subject} = ${subject_base64} ] ; then
    ...
    message_urlencoding=$(cat message.txt | python3 -c "import sys, urllib.parse; print (urllib.parse.quote(sys.stdin.read().rstrip()));")
